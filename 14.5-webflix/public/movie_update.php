...
$released_at = $_POST['released_at'] ?? $movie['released_at'];
$categorySelected = $_POST['category'] ?? $movie['category_id'];
...
if (!empty($_POST)) {
    ...
    if (empty($errors)) {
        $query = $db->prepare(
            'UPDATE movie SET title = :title, description = :description, cover = :cover, duration = :duration, released_at = :released_at, category_id = :category_id WHERE id = '.$id
        );
